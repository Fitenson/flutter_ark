...
class ArkApp extends StatefulWidget {
  /// Creates an [ArkApp] providing a [ArkTheme].
  const ArkApp({
    super.key,
    this.navigatorKey,
    ...
    this.scrollBehavior = const ArkScrollBehavior(),
  ...
  /// Creates an [ArkApp] that uses the [Router] instead of a [Navigator].
  const ArkApp.router({
    super.key,
    this.theme,
    this.darkTheme,
  ...
  const ArkApp.custom({
    ...
    this.navigatorObservers = const []
        ...
        scrollBehavior = const ArkScrollBehavior(),
        shortcuts = null,
        ...
        title = '',
        type = ArkAppType.custom;

  /// The type of app to use.
  ///
  /// If you are going to use only Ark components, use [ArkAppType.ark].
  /// If you are going to use also Material or Cupertino components, use [ArkAppType.custom].
  final ArkAppType type;

  /// Default visual properties, like colors fonts and shapes, for this app's
  /// shad widgets.
  ///
  /// A second [darkTheme] [ArkThemeData] value, which is used to provide a
  ...
  /// `ArkThemeData(brightness: Brightness.light)`.
  final ArkThemeData? theme;

  /// The [ArkThemeData] to use when a 'dark mode' is requested by the
  /// system.
  ///
  ...
  ///
  /// This theme should have a [ArkThemeData.brightness] set to
  /// [Brightness.dark].
  ///
  /// Uses [theme] instead when null. Defaults to the value of
  /// [ArkThemeData(brightness: Brightness.light)] when both [darkTheme] and
  /// [theme] are null.
  ...
  /// you want to add between the [ArkApp] and its child and get the same
  /// effect.
  ///
  /// ```dart
  ...
  /// The curve used to animated from one [ArkThemeData] to another, defaults
  /// to [Curves.linear].
  ...
  /// {@macro ArkAppBuilder.backgroundColor}
...
class _ArkAppState extends State<ArkApp> {
  ...
  ArkThemeData get effectiveLightTheme =>
      widget.theme ??
          ArkThemeData(
...
/// Describes how [Scrollable] widgets behave for [ArkApp]s.
///
...
class ArkScrollBehavior extends ScrollBehavior {
  /// Creates a ArkScrollBehavior that decorates [Scrollable]s with
  /// [Scrollbar]s based on the current platform and provided
  /// [ScrollableDetails].
  const ArkScrollBehavior();
...
class ArkAppBuilder extends StatelessWidget {
  ...
  /// {@template ArkAppBuilder.backgroundColor}
  /// The background color of the page, defaults to `ArkTheme.of(context).colorScheme.background`.
  /// {@endtemplate}
  final Color? backgroundColor;
  ...
  Widget build(BuildContext context) {
    ...
    return Builder(
      builder: (BuildContext context) {
        // The reason for wrapping the [child] with `ArkToaster` and `ArkSonner`
        ...
        final effectiveChild = child != null
            ? ArkToaster(child: ArkSonner(child: child!))
            : child;
        return ColoredBox(
          color:
          backgroundColor ?? ArkTheme.of(context).colorScheme.background,
